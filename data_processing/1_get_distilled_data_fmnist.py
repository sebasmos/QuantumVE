...
def main():
    parser = argparse.ArgumentParser(description="Quantum SVM Training with TN-SM and Distillation on FashionMNIST")
    parser.add_argument("--output_dir", type=str, default="../data/fashionmnist_embeddings/",
    ...
    train_dataset = FashionMNIST(root=f"{args.output_dir}/raw", train=True, download=True, transform=transform)
    images_tensor = train_dataset.data.float() / 255.0
    images_np = images_tensor.numpy()
    labels = train_dataset.targets.numpy().astype(int)
    flattened_images = images_np.reshape(len(images_np), -1)

    # Load FashionMNIST test data
    test_dataset = FashionMNIST(root=f"{args.output_dir}/raw", train=False, download=True, transform=transform)
